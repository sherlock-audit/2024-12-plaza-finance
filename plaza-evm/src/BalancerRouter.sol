...
contract BalancerRouter is ReentrancyGuard {
    ...
    function joinBalancerPool(
        ...
        for (uint256 i = 1; i < assets.length; i++) { // index 0 is the balancer pool token in ManagedPools
    ...
    function exitPlazaAndBalancer(
        ...
        if (exitKind == 0) { // EXACT_BPT_IN_FOR_ONE_TOKEN_OUT
        ...
        } else if (exitKind == 1) { // EXACT_BPT_IN_FOR_TOKENS_OUT
