...
contract Pool is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable, OracleReader, Validator {
  ...
  uint256 private constant AUCTION_START_BLOCK_BUFFER = 3;
  ...
  uint256 public lastAuctionStartBlock;
  uint256 private lastCreateBlock;
  ...
  function startAuction() external whenNotPaused() noCreateInThisBlock {
    ...
    lastAuctionStartBlock = block.timestamp;
  }
  ...
  modifier passedRecentAuctionStart() {
    if (lastAuctionStartBlock + AUCTION_START_BLOCK_BUFFER > block.number) {
      revert AuctionRecentlyStarted();
    }
    _;
  }

  modifier noCreateInThisBlock() {
    if (lastCreateBlock == block.number) {
      revert CreateInThisBlock();
    }
    _;
  }
}
